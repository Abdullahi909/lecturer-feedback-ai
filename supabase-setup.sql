-- Enable UUID generation so the seed rows can use fixed ids.
create extension if not exists pgcrypto;

-- Drop old demo tables first so the script can be run again safely.
drop table if exists submissions;
drop table if exists modules;
drop table if exists users;

-- Create the simple users table.
create table users (
...
-- Turn off RLS to keep the demo simple.
alter table users disable row level security;
alter table modules disable row level security;
alter table submissions disable row level security;

-- Add one lecturer and several students.
...
-- Add the modules used in the demo.
...
-- Add some example submissions so the pages are not empty on first run.
